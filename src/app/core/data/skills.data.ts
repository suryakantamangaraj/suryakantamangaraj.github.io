import { ISkill } from '../models/skill.interface';

export const SKILLS_DATA: ISkill[] = [
  {
    type: 'EDA Tools',
    names: [
      'Cadence OrCAD', 'Xilinx Vivado', 'MATLAB', 'Cadence Analog Workbench', 
      'Cadence Virtuoso', 'Altium Designer', 'LTSpice'
    ]
  },
  {
    type: 'Computer Coding Languages',
    names: [
      'Python', 'C', 'MATLAB', 'Verilog', 'JavaScript', 'TypeScript', 'Shell Scripting'
    ]
  },
  {
    type: 'Automation Tools',
    names: [
      'NI LabVIEW', 'DAQ Systems', 'Selenium'
    ]
  },
  {
    type: 'T&M Tools',
    names: [
      'Spectrum Analyzers', 'Logic Analyzers', 'Digital Meters', 'Safety Testers', 
      'Signal Generators', 'DSOs', '& Other Tools'
    ]
  },
  {
    type: 'Others',
    names: [
      'Microsoft Office', 'VCS', 'CI/CD', 'Test Management', 'Project Scoping'
    ]
  }
];